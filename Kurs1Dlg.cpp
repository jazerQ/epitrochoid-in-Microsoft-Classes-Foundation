...
BEGIN_MESSAGE_MAP(CKurs1Dlg, CDialogEx)
	...
	ON_EN_CHANGE(IDC_EDIT_R, &CKurs1Dlg::OnEnChangeEditR)
...
BOOL CKurs1Dlg::OnInitDialog()
{
	...
	m_R = uA;
	CString strA;
	strA.Format(L"%f.0", m_R);
	m_EdR.SetWindowTextW(strA);
	m_bInited = true;
...
void CKurs1Dlg::OnBnClickedOk()
{
	...
	AfxGetApp()->WriteProfileInt(L"Settings", L"ParamA", (UINT)m_R);
	CDialogEx::OnOK();
}


void CKurs1Dlg::Risovat(double R, double Rk, double H)
{
	m_Calc.SetPar(R, Rk, H);
...
void CKurs1Dlg::OnEnChangeEditR()
{
	CString strA;
	m_EdR.GetWindowText(strA);
	m_R = _wtof(strA);
...
void CKurs1Dlg::OnEnChangeEditRk()
{
	CString strA;
